...
- [验收操作指南](#验收操作指南)
  - [开关功能总览](#开关功能总览)
  - [一、Project 1（单周期 SCPU）验收](#一project-1单周期-scpu验收)
  - [二、Project 2（流水线 PCPU）验收](#二project-2流水线-pcpu验收)
  - [三、验收速查表](#三验收速查表)
...
| 开关 | 接到哪 | 作用 |
|------|--------|------|
| `SW0` | `SSeg7.SW0` | 切换数码管显示模式：`0`=纯 hex；`1`=测试结果模式（前两位显示 AC） |
| `SW2` | `clk_div.SW2` | CPU 时钟档：`0`=快档 `clkdiv[3]`（≈6 MHz）；`1`=慢档 `clkdiv[24]`（≈3 Hz，能肉眼看 PC 跑动） |
| `SW7,SW6,SW5` | `Multi_8CH32.Switch[2:0]` | 选择 8 路 32bit 信号中的哪一路送到数码管显示 |

`Multi_8CH32` 的 8 个数据通道对应关系：

| SW7 SW6 SW5 | 通道 | 显示内容 | 用来看什么 |
|-------------|------|----------|------------|
| 0 0 0 | `data0 = Peripheral_in` | 最近一次写入 `0xe0000000` 的数据 | 测试进度码（`0x00111100`、…、`0x00666600`） |
| 0 0 1 | `data1 = {2'b00, PC[31:2]}` | PC 的字地址（`PC>>2`） | 跟踪指令编号 |
| 0 1 0 | `data2 = spo` | 当前 ROMD 输出的指令机器码 | 当前正在执行的 32bit 指令 |
| 0 1 1 | `data3` | Project1=浮空；Project2=`counter_out` | Project2 看硬件计数器 |
| 1 0 0 | `data4 = Adder_out` | ALU 结果 / 访存地址 | `lw/sw` 的目标地址或 ALU 输出 |
| 1 0 1 | `data5 = Data_out` | CPU 输出的写数据（`rs2_data`） | `sw` 写出去的内容 |
| 1 1 0 | `data6 = Cpu_data4bus` | 总线读回的 32bit 字 | `lw` 读到了什么 |
| 1 1 1 | `data7 = PC_out` | PC 原值（字节地址） | 真实 PC，单步对照反汇编 |

LED 显示的是程序写到 `0xf0000000` 的低 16 位，与数码管进度码同步。

---

### 一、Project 1（单周期 SCPU）验收

使用 `testac.coe`（指令）+ `D_mem.coe`（数据）。

#### 1.1 总体验收（必做）

| 步骤 | SW 设定 | 期望现象 | 验什么 |
|------|---------|----------|--------|
| 1 | `SW0=1`、`SW7..SW5=000`、`SW2=0`（快档），按 rstn 复位 | 数码管显示 **AC123456** | 整个 CPU 全功能通过，6 个测试函数全部 OK |
| 2 | 同上但 `SW2=1`（慢档） | 进度码依次跳过 `00111100 → 00222200 → … → 00666600`，切 `SW0=1` 后出 **AC123456** | 肉眼看到每个子测试依次通过 |
| ❌ 失败 | — | 后 6 位出现 `0F00xxxx` 或前两位不是 AC | `xxxx` 低 4×N 位告知卡在第几个测试之前 |

**AC 显示原理回顾：** 每个测试通过都把测试号压进 `mem[4]`（左移 4 + OR n），全部通过后 `mem[4] = 0x123456`；`SSeg7` 在 `SW0=1` 时把前 2 位强制画成 "AC" + 后 6 位画 `mem[4]`。失败时程序跳到 `0x204` 死循环，把 `0xf0000000 | mem[4]` 写到数码管，出现 `0xF` 前缀即为失败标志。

#### 1.2 细分演示（单步验 CPU 内部）

`SW2=1`（慢档）+ `SW0=0`（纯 hex 模式），切换 `SW7~SW5` 对照 `rv32-instr.disasm` 逐项核查：

| SW7 SW6 SW5 | 看到的内容 | 验什么 |
|-------------|------------|--------|
| 0 0 0 | 测试进度码（`0x111100→…→0x666600`） | 进度码按时序变化 = 各 test 通过 |
| 0 0 1 | PC>>2 字地址 | "下一条指令在第几号"，便于对反汇编 |
| 0 1 0 | 当前指令机器码 | 例如 `ff010113`（PC=0 时第一条 `addi sp,sp,-16`），证明 ROMD 取指正确 |
| 1 0 0 | ALU 输出 | 验 ADD/SUB/SLT/XOR 等运算结果 |
| 1 0 1 | rs2 写出数据 | 验 `sw` 时确实写出了正确数据 |
| 1 1 0 | 总线读回数据 | 验 `lw` 时读回正确（含 `lh/lhu/lb/lbu` 字节/半字提取） |
| 1 1 1 | PC 原值（字节地址） | 看到 PC 在跳转处直接跳到 `jal/jalr/branch` 的目标 |

---

### 二、Project 2（流水线 PCPU）验收

PCPU_TOP 接口与 SCPU_TOP 完全一致，所有显示规则继续有效。流水线版需额外验证：（A）能正确跑通 RV32I 测试集；（B）流水线特有的数据通路和 IO 演示。

#### 2.1 配置 A：`testac.coe` + `D_snakeDEMO.coe` — 验流水线功能正确性

操作与 Project 1 的 1.1 完全一致：

| 步骤 | SW 设定 | 期望现象 | 验什么 |
|------|---------|----------|--------|
| 1 | `SW0=1`，`SW7..5=000`，`SW2=0` | 数码管显示 **AC123456** | PCPU 五段流水线 + 前递 + 冒险处理 + Load-Use Stall + 分支冲刷全部正确 |
| 2 | `SW0=0`，`SW2=1`，`SW7..5=001` | PC 字地址逐条递增 | 慢档下 PCPU 跑完一轮比 SCPU 明显更快（IPC≈1，分支/load-use 损失拍数较少） |
| 3 | `SW0=0`，`SW2=1`，`SW7..5=011` | `counter_out` 数值在变化 | Project2 多接了计数器通道，验证 Counter_x 通道 0 在正常工作 |

**失败诊断：** 若出现 `0xF000xxxx` 而 SCPU 跑同样的 testac 正常，几乎一定是流水线冒险处理遗漏——最常见场景：分支前 1 条 ALU 指令依赖前 2 条 ALU 指令（需走 EX/MEM 前递），或 `jalr` 的目标地址来自 `lw` 读出的结果。

#### 2.2 配置 B：`I_pipemem37.coe` + `D_snakeDEMO.coe` — 贪吃蛇 IO 演示

将 ROMD 的 COE 换成 `I_pipemem37.coe`，RAM_B 用 `D_snakeDEMO.coe`。

| 步骤 | SW 设定 | 期望现象 |
|------|---------|----------|
| 1 | `SW0=0`，`SW2=0`（快档），`SW7..5=000` | 数码管循环刷新——程序按帧将 `D_snakeDEMO` 中的模式数据写到 `0xe0000000`，8 位数码管点亮模式形成蛇身延伸/缩短的视觉动画 |
| 2 | — | LED 同步按帧切换（程序同时写 `0xf0000000`） |
| 3 | `SW7..5=110` | `data6 = Cpu_data4bus`，可看每次 `lw` 读出的快照，验证读到的是 `D_snakeDEMO` 里的模式数据 |
| 4 | `SW7..5=001` | PC 字地址快速循环，证明程序在 IO 测试主循环里反复 `jal` |

> `icf.xdc` 已约束好 16 个 LED、8 位数码管、16 个拨码开关、5 个按键的全部引脚，图案直接显示在板上，无需额外配置。

#### 2.3 配置 C（可选）：`testac.coe` + `D_snakeDEMO.coe`

结果与配置 A 相同，显示 **AC123456**。`D_snakeDEMO` 多出的图案数据 testac 程序不使用，不会自动出现贪吃蛇动画（图案需要由程序主动读取并写出 GPIO 才会"动"）。

---

### 三、验收速查表

| 想验的内容 | 项目 | COE 组合 | SW 设定 | 通过标志 |
|------------|------|----------|---------|----------|
| 单周期 CPU 全功能 | Project 1 | `testac` + `D_mem` | `SW0=1`，`SW7..5=000` | **AC123456** |
| 流水线 CPU 全功能 | Project 2 | `testac` + `D_snakeDEMO` | `SW0=1`，`SW7..5=000` | **AC123456** |
| 贪吃蛇 IO 演示 | Project 2 | `I_pipemem37` + `D_snakeDEMO` | `SW0=0`，`SW2=0`，`SW7..5=000` | 数码管/LED 出贪吃蛇动画 |
| 单步看 PC | 任意 | 任意 | `SW2=1`，`SW0=0`，`SW7..5=111` | PC 缓慢递增，能对照反汇编 |
| 看当前指令 | 任意 | 任意 | `SW2=1`，`SW0=0`，`SW7..5=010` | 显示反汇编对应行的机器码 |
| 看 `lw` 读回值 | 任意 | 任意 | `SW0=0`，`SW7..5=110` | hex = 该地址的数据内容 |
| 看 `sw` 写出值 | 任意 | 任意 | `SW0=0`，`SW7..5=101` | hex = rs2 的数据 |
| 看 ALU 结果 | 任意 | 任意 | `SW0=0`，`SW7..5=100` | 例如 `add` 后 = 两操作数之和 |
| 看测试进度 | 任意 | testac | `SW0=0`，`SW2=1`，`SW7..5=000` | `0x111100 → … → 0x666600` |
| 看硬件计数器 | Project 2 | 任意 | `SW7..5=011` | `counter_out` 数值在递减 |

> **核心一句话：** 验收主要看 `SW0=1`，`SW7..5=000` 是否出 "AC"——出 AC 即 CPU 通过 RV32I 全套自检；其他 SW 组合是给老师/助教"打开 CPU 的盖子看内部"用的细分演示。
